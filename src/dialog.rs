...
impl_actions!(dialog, [DialogResponse]);
...
impl Dialog {
    ...
    pub fn show(&mut self, request: DialogRequest, subscription: Option<Subscription>, cx: &mut ViewContext<Self>) {
    ...
    fn bind_keys(&mut self, cx: &mut ViewContext<Self>) {
        println!("Rebinding keys for dialog-mode");
        cx.clear_key_bindings();
        cx.window_context().bind_keys(self.actions.iter().enumerate().map(|(idx, a)| {
            KeyBinding::new(&a.key, DialogResponse::new(idx, None), None)
        }));

        if !self.options.is_empty() {
            cx.window_context().bind_keys([
    ...
    pub fn show_just_error(&mut self, msg: SharedString, cx: &mut ViewContext<Self>) {
        self.show(
...
impl Render for Dialog {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        ...
        if !self.options.is_empty() {
            ...
            content = content.child(uniform_list(cx.view().clone(), "dialog_options", self.options.len(), |this, range, cx| {
                let mut items = Vec::new();
                let app_global = cx.global::<AppGlobal>();
                for idx in range {
                    let img_src = app_global.match_icon(this.options[idx].icon_name.as_str(), 32, cx.scale_factor())
        ...
        content = content.child(div().flex().flex_row().justify_center().children(self.actions.iter().enumerate().map(|(idx, action)| {
            div().border_1().border_color(rgb(0x787878)).cursor_pointer()
                .px_2().m_1()
                .on_mouse_up(MouseButton::Left, cx.listener(move |this, _, cx| cx.emit(DialogResponse::new(idx, this.sel_option.clone()))))
        ...
        d = d.on_action(cx.listener(|this, a: &DialogResponse, cx| {
        ...
        if !self.options.is_empty() {
            d = d.on_action(cx.listener(|this, _: &DialogNextOption, cx| {
                this.sel_option = Some(this.sel_option.map(|sel| std::cmp::min(sel + 1, this.options.len() - 1)).unwrap_or(0));
                cx.notify();
            }));
            d = d.on_action(cx.listener(|this, _: &DialogPrevOption, cx| {
